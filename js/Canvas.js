...
class Canvas{
    ...
    constructor(stationName, stationNombr){
        ...
        this.nombr = stationNombr;
        this.ajoutCanvas();
        window.canva = this;
        this.canvas = document.getElementById('myCanvas');// On accéde à l’élément id "canvas" en JavaScript
...
//méthode pour ajouter le HTML du canvas
    ajoutCanvas() {
        document.getElementById("myCanvasSection").innerHTML = "<form id='canvasForm'><p>Vous êtes sur le point de réserver un vélo à la station: "+this.name+".<p><p>Merci de bien vouloir signer:</p><canvas id='myCanvas'></canvas><div id='inputCanvas'><input type='submit' id='confirmCanvas' value='Confirmer'><input type='reset' id='resetCanvas' value='Effacer'></div></form>"; 
    }
...
//méthode pour réinitialiser le canvas    
    refreshCanvas() {
        ...
        document.getElementById("myCanvasSection").scrollIntoView({behavior: 'smooth', block: 'start'});        
    }
...
//méthode pour vérifier si canvas signé avec ou sans révervation et denamder de signé avec message d'alerte    
    submitReservation() {
        ...
        if (window.canva.signed){
            ...
            let time = Date.now();//La méthode Date.now() renvoie le nombre de millisecondes écoulées depuis le 1er Janvier 1970 00:00:00 UTC.
            let reservation = new Reservation(window.canva.name, time); 
            document.getElementById("myStationSection").innerHTML = "";//on reste sur à la carte
            document.getElementById("myCanvasSection").innerHTML = "";
            location.reload();//on recharge la location    
        }

        //si non signé demander la signature via une alert et ramener au canvas
        else {
